...
CuotasBorraDeltas <- function(dataset)
{
  #se borran las variables lag1
  dataset[, grep(".*lag1", colnames(dataset), perl=TRUE, value=TRUE):=NULL]
  # se borran la variable delta1
  dataset[, grep(".*delta1", colnames(dataset), perl=TRUE, value=TRUE):=NULL]
  ReportarCampos( dataset )
}

#------------------------------------------------------------------------------
#ranking - 2 opciones ranking comun y normalizado

Ranking_comun <- function( dataset, cols )
{
  dataset[, paste0( cols, "_rank") := lapply( .SD, frankv, na.last="keep", ties.method="dense" ), by= foto_mes, .SDcols= cols]
  ReportarCampos( dataset )
}
...
correr_todo  <- function( palancas )
{
  ...
  cols_cuotas<-c("mprestamos_hipotecarios","mprestamos_personales","mprestamos_prendarios")
  if( palancas$agregar_cuotas )   Lags( dataset, cols_cuotas, 1, 1 )
  if ( palancas$agregar_cuotas) Cuotas(dataset)
  if ( palancas$agregar_cuotas) CuotasBorraDeltas(dataset)
